...
void help();
void assure(const int testID, Board& b, const int depth, const size_t expected, const bool print_time);
...
int main(int argc, char **argv) {
   ...
   for (int i = 1; i < argc; ++i) {
      if (string(argv[i]) == "-h" || string(argv[i]) == "--help")
         help(), exit(0);
      else if (string(argv[i]) == "-e" || string(argv[i]) == "--extended")
         extended_tests = true;
      else help(), exit(1);
   }
...
void assure(const int testID, Board& b, const int depth, const size_t expected, const bool print_time) {
   const auto generated{ print_time ? timed_perft(b, depth) : perft(b, depth) };
