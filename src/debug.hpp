...
inline string pt_to_str(const PieceType pt) {
  if (pt >= 6)
    return "";
  return array{"Knight", "Bishop", "Rook", "Queen", "King", "Pawn"}[pt];
}

std::string sq_to_str(Square sq) {
  if (sq == NoSquare) {
    return "No Square";
  }
  else {
...
[[maybe_unused]] static void print_board(Board &b, PieceColor p = NoColor) {
  for (int i = 7; i >= 0; --i) {
    for (int j = 0; j < 8; ++j) {
...
[[maybe_unused]] inline string sqstr(const Square sq) {
  string str = "a1";
  str[0] += sq % 8;
  str[1] += sq / 8;
  return str;
...
[[maybe_unused]] inline string str_repeat(const string &str, const int n) {
  string res = "";
  for (int i = 0; i < n; ++i)
    res += str;
  return res;
