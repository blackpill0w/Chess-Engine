...
Board::Board(const string &FEN) : piecesBB{}, move_history{}, movelist{}, zobrist{} {
...
PieceBBIndex Board::get_pieceBB_index(const Square sq) const {
  for (size_t i = 0; i < piecesBB.size(); ++i)
    if (sqbb(sq) & piecesBB.at(i))
      return PieceBBIndex(i);
...
bool Board::is_promotion(const Square from, const Square to) const {
  const bool is_white_promotion = (sqbb(from) & piecesBB[WP]) && (get_rank(to) == rank8);
  const bool is_black_promotion = (sqbb(from) & piecesBB[BP]) && (get_rank(to) == rank1);
  return (is_white_promotion || is_black_promotion);
}
...
Bitboard Board::attackers_of(const Square sq) const {
  ...
  res |= gen_sliding_piece_moves(sq, Rook, all_pieces(), get_pieces(myc, NoType)) &
...
Bitboard Board::gen_piece_moves(const Square sq, const Bitboard occ,
                                const Bitboard mypieces) const {
  Bitboard res         = 0;
...
void Board::gen_board_legal_moves() {
  movelist.clear();
  if (state == Playing && fifty_move_counter == 100)
    state = Draw;
  ...
  if (state == Playing && (fifty_move_counter == 100 || is_draw_by_repetition()))
    state = Draw;
}

bool Board::is_valid_move(const Square from, const Square to) const {
  auto x = find_if(movelist.begin(), movelist.end(), [&](const PieceMoves &pm) {
    return pm.pos == from && (sqbb(to) & pm.possible_moves);
  });
...
BoardErr Board::make_move(const Square from, const Square to, const PieceType promote_to) {
  ...
  // Handle promotion
    ...
    assert(promote_to == Queen || promote_to == Rook || promote_to == Bishop ||
           promote_to == Knight);
...
BoardErr Board::unmake_move() {
  ...
  if (md_get_move_type(md) == Promotion) {
    ...
    piecesBB[get_pieceBB_index(Pawn, ~color_to_play)] |=
        sqbb(to - 8 * pawn_direction(color_to_play));
    enpassant_square = to;
  }
  else if (md_get_move_type(md) == Castling) {
...
Key Board::calc_zobrist_key() {
  ...
  if (enpassant_square != NoSquare)
    k ^= Zobrist::enpassant.at(get_file(enpassant_square));
  // Castling rights
  if (cr & White_OO)
    k ^= Zobrist::castling_rights[0];
  if (cr & White_OOO)
    k ^= Zobrist::castling_rights[1];
  if (cr & Black_OO)
    k ^= Zobrist::castling_rights[2];
  if (cr & Black_OOO)
    k ^= Zobrist::castling_rights[3];
  // Pieces
...
bool Board::is_draw_by_repetition() const {
  ...
  for (int i = last_irreversible_move + 1; i < zobrist.size(); i++) {
    if (zobrist.at(i) == zobrist.at(lastkey) && ++cnt == 3)
      return true;
  }
  return false;
}
