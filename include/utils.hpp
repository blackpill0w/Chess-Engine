...
using Key      = uint64_t;  // Zobrist Key
...
// clang-format on
inline constexpr PieceColor operator~(PieceColor c) { return PieceColor(c ^ 1); }
inline constexpr int pawn_direction(PieceColor c) { return c == White ? 1 : -1; }
...
inline constexpr bool squares_on_same_file(Square sq1, Square sq2) {
  return get_file(sq1) == get_file(sq2);
}
// clang-format off
enum Rank
{
...
inline constexpr bool squares_on_same_rank(Square sq1, Square sq2) {
  return get_rank(sq1) == get_rank(sq2);
}
// clang-format off
enum Direction
{
